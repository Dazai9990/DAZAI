...
class sms_man_com { "7274696715:AAFzdJ-Jb3a9pItpifMhcf1SaMowckfUkMI"
	
	//private $api_key;
	private $link = "https://smslive.pro/stubs/handler_api.php";
	...
	public function getNumber ($co,$ser) {
		$get = file_get_contents($this->link ."&action=getNumber&service=wa&country={$co}");
		$ex = explode (":",$get); 
		$id = $ex[1]??0;
		$number = $ex[2]??0;
		return $this->isError ($get) ? 
