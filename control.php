...
if($data == 'back_' || preg_match("/^back_[^\_]+$/",$data)){
	$bot->editMessageText([
	'chat_id'=>$chat_id,
	'text'=>str_replace('++DN++',$first_name,$ow_panel['texts']['startAM']),
	'parse_mode'=>'MarkDown',
...
if($data == 'status'){
	...
	$keyboard     = make_buttons(['back_'=>$back_home['back_']]);
	$bot->editMessageText([
	'chat_id'=>$chat_id,
	'text'=>$resua,
	'parse_mode'=>'MarkDown',
	...
	if($Status[$chat_id]['mail'] != null){
		if(preg_match("/^\d+$/",$text)){
			...
			$bot->sendMessage([
		       ...
		       'chat_id'=>$connect->getOwner(),

		       'text'=>"✅ تم شحن حسابك مبلغ".$text." روبل
↩️رصيدك  الان  ".$connect->getBalance(),
		       ]);
			unset($Status[$chat_id]);
			$timed_db->setData($Status);
		} else {
...
if(preg_match("/^setApiFor\_(.+)$/",$data,$infP)){
	...
	$bot->editMessageText([
	'chat_id'=>$chat_id,
	'text'=>$ow_panel['texts']['setApiFor'].$theSite.$artical,
	'parse_mode'=>'MarkDown',
	'message_id'=>$message_id,
	'reply_markup'=>json_encode($sitesArray)
	]);
...
if($data == 'setJoin'){
	...
	if($chNow == true && $chNow != '@ALBADRPLUS'){
