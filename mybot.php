...
if($text){
	...
	if($myChannel == true && $myChannel != '@ALBADRPLUS'){
...
if (array_key_exists($data,$reace_rand)){
	...
	if ($opreag == 'done'){
		...
		$rosk = rand(1000000,999999);
		$respi    = ['++P++','++IDT++','++NUMBER++','++CODE++','++AN++'];
		$resbi    = [$price,$rosk,$Number,'*.....*',$add_apps[substr($app_code,0,2)]];
...
$ICX    = [];
		if(substr($app_code,0,2) == 'tg'){
			$ICX["https://t.me/{$Number}"]  = $buttons_buy['view_on'];
		}
...
for($xn=0;10>$xn;$xn++){
	...
	$rosk = rand(1000000,999999);
	$respi    = ['++P++','++IDT++','++NUMBER++','++CODE++','++AN++'];
	$resbi    = [$price,$rosk,$Number,'*.....*',$add_apps[$app_code]];
	$respo   = str_replace($respi,$resbi,$text_buy['buy']);
...
if ($plant == false){
	$keyboard     = make_buttons(["{$data}"=>'🏆 الشراء 10x مجددا']);
	$bot->sendMessage([
	'chat_id'=>$chat_id,
	'text'=>'',
	'reply_markup'=>json_encode($keyboard)
...
if(preg_match("/^buyNumber_(\w+)_(\w+)_((.+)#(.+))@.$/",$data,$dConfig)){
	...
	if (is_array($Status['timebuy']) and $Status['timebuy'][$chat_id] != null){
		if (3 > (time() - $Status['timebuy'][$chat_id])){
			$bot->answerCallBackQuery([
			'callback_query_id'=>$update->callback_query->id,
			'text'=>'لحظة لحظة...',
			'show_alert'=>true,
	...
	if($get_number[0] == false){
		...
		$keyboard     = make_buttons(["try{$data}"=>'🏆 شراء 10x']);
		$bot->sendMessage([
		'chat_id'=>$chat_id,
		'text'=>'',
		'reply_markup'=>json_encode($keyboard)
